...
function placeDiv() {
    for (let i=0; i<catalogue.length; i++){
        ...
        document.querySelector("#from"+i).insertAdjacentHTML("beforeend",`
            ...
            <p>${note}</p>
            <p>Type : ${catalogue[i].type}<br> Planete : ${catalogue[i].planete}<br>Attaque :${catalogue[i].attaque}</p>`);
        document.querySelector("#from"+i).onclick = function(){
            this.style.background = "red";
            this.style.marginLeft = " 10%" ;
            document.querySelector("article").style.background = " url('db"+i+".jpg') no-repeat";
...
function fond(){
    ...
    for (let i=0;i<div.length;i++){
        ...
        div[i].style.marginLeft = " 3.5%" ;
    }
    document.querySelector("article").style.background = " url('db.jpg') no-repeat";
    document.querySelector("article").style.backgroundSize = " cover ";
}
